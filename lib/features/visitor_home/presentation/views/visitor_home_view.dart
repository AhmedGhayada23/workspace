...
class VisitorHomeView extends GetView<VisitorHomeController> {
  ...
  Widget build(BuildContext context) {
    final controller = Get.put(VisitorHomeController());
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          child: Stack(
                            ...
                            children: [
                              Center(child: SvgPicture.asset(AppSvg.notificationSvg)),
                              LocalStorage().readValue<bool>(Constants.unreadNotification) == true
                                  ? Positioned(
                                    top: 10,
                                    right: 7,
                                    child: CircleAvatar(
                ...
                () =>
                    controller.loading.isTrue
                        ...
                        ? SingleChildScrollView(
                          ...
                          child: Row(
                            children: List.generate(
                              controller.listNewSpacesData.length,
                              (index) => FadeInUp(
                                child: Padding(
                                  padding: EdgeInsets.only(right: 16.w),
                                  child: ItemSpaceWidget(
                                    ...
                                    typeTitle:
                                        controller.listNewSpacesData[index].company?.typeTitle ??
                                        '-',
                                    image: controller.listNewSpacesData[index].mainImageUrl ?? '',
                                    nameCompany:
                                        controller.listNewSpacesData[index].company?.name ?? '-',
                                    ratingCount:
                                        '${controller.listNewSpacesData[index].ratingCount ?? '0'}',
                                    ratingAverage:
                                        '${controller.listNewSpacesData[index].ratingAverage ?? '0'}',
                                    address: controller.listNewSpacesData[index].address ?? '-',
                                    available:
                                        '${controller.formatTime(controller.listNewSpacesData[index].availableFrom)} - ${controller.formatTime(controller.listNewSpacesData[index].availableTo)}',
                ...
                () =>
                    controller.loading.isTrue
                        ...
                        ? SingleChildScrollView(
                          child: Column(
                            children: List.generate(
                              controller.listsuggestSpacesData.length,
                              (index) => FadeInUp(
                                child: Padding(
                                  padding: EdgeInsets.only(left: 16.w, right: 16.w),
                                  child: AllItemSpaceWidget(
                                    ...
                                    typeTitle:
                                        controller
                                            .listsuggestSpacesData[index]
                                            .company
                                            ?.typeTitle ??
                                        '-',
                                    image:
                                        controller.listsuggestSpacesData[index].mainImageUrl ?? '',
                                    nameCompany:
                                        controller.listsuggestSpacesData[index].company?.name ??
                                        '-',
                                    ratingCount:
                                        '${controller.listsuggestSpacesData[index].ratingCount ?? '0'}',
                                    ratingAverage:
                                        '${controller.listsuggestSpacesData[index].ratingAverage ?? '0'}',
                                    address: controller.listsuggestSpacesData[index].address ?? '-',
                                    available:
                                        '${controller.formatTime(controller.listsuggestSpacesData[index].availableFrom)} - ${controller.formatTime(controller.listsuggestSpacesData[index].availableTo)}',
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(padding: EdgeInsets.only(left: 16.w, right: 16), child: FilterVisitorHomeWidget()),
          ],
        ),
      ),
    );
